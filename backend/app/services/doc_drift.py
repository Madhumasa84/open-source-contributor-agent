...
class DocDriftDetector:
    ...
    async def detect_and_report(self, repo_url: str, patch_diff: str, files_changed: list[str]):
        ...
        if not has_doc_changes and len(patch_diff.split("\n")) > 50:
            ...
            # Since bot currently only has apply_labels and post_comment,
            # we add create_issue directly here or extend the bot.
            ...
            try:
                ...
                changed_str = ", ".join(files_changed)
                body = (
                    "🤖 **OSCA Doc Drift Detector**\n\n"
                    f"A recent patch by OSCA modified logic in `{changed_str}` "
                    "significantly without updating corresponding documentation or docstrings. "
